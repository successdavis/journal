<template>
    ...
    <div class="relative max-w-2xl w-full h-full flex items-center justify-center">
        <!-- Notification Icon -->
        <button class="absolute z-40" @click="displayNotificationMessages">
            ...
            <span
                ...
                class="absolute -top-4 -right-3 transform -translate-x-1/2 bg-red-600 text-white text-xs px-1.5 py-0.5 rounded-full"
            >
        {{ notificationsCount }}
      </span>
        </button>
        ...
        <div
            :class="[
        'absolute top-6 left-1/2 transform -translate-x-1/2 z-50 h-fit',
        showMessages ? '' : 'hidden'
      ]"
        >
            <div class="bg-white p-4 rounded shadow w-80">
                ...
                >
                    <li
                        ...
                        :key="index"
                        class="break-words flex items-start justify-start"
                    >
                        📢 <span
                        :class="[ 'px-2',
...
const getNotifications = () => {
    axios.get(`/get_notifications/${props.user.id}`).then((res) => {
        notifications.value = res.data.notifications
        notificationsCount.value = res.data.notificationsCount
    })
}

const displayNotificationMessages = () => {

    axios.patch(`/notifications_read/${props.user.id}`).then((res) => {
