<template>
    <li>
        <button type="button"
                @click="active = !active"
                class="flex items-center p-2 w-full text-base font-normal rounded-lg transition duration-75 group hover:text-gray-700 hover:bg-gray-100"
                aria-controls="dropdown-example" data-collapse-toggle="dropdown-example">

         <slot name="icon"></slot>

            <span class="flex-1 ml-3 text-left whitespace-nowrap" sidebar-toggle-item>{{ name }}</span>
            <svg sidebar-toggle-item class="w-6 h-6 transition-all" :class="active ? 'close-accordian transition-all' : '' " fill="currentColor" viewBox="0 0 20 20"
        ...
        <ul id="dropdown-example" class=" py-2 space-y-2 transition-all" :class="active ? '' : 'hidden transition'">
            <slot></slot>
        </ul>
    </li>
</template>
