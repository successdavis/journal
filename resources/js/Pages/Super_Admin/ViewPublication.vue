    ...
    <span class="inline-block bg-green-100 text-green-800 text-sm font-medium px-3 py-1 rounded-full">
      Free Article
          ...
          >
            ...
            <!-- Additional Files -->
            <div v-if="supplementary.length || figures.length || publication.cover_letter">
                ...
                <ul class="list-disc flex justify-between list-inside text-sm text-gray-700 dark:text-gray-300 space-y-1">
                    ...
                    <li class="flex flex-col" v-for="(fig, index) in figures" :key="fig">
                        <a :href="`/storage/${fig}`" class="text-blue-600 hover:underline" target="_blank">Figure:
                            {{ index + 1 }}</a>
                    </li>
        ...
        <div v-if="$page.props.auth.user.role  === 'Super_Admin' || $page.props.auth.user.role  === 'Editor'">
            <AssignReviewerSection
                :itemId="publication.id"
            />
        </div>

        <div v-if="$page.props.auth.user.role  === 'Reviewer'">
            <ReviewActionByReviewer
                :item="publication"
            />
        </div>
    </div>
...
const keywords = props.publication.keywords?.split(',') || []
const figures = JSON.parse(props.publication.figures || '[]')
const supplementary = JSON.parse(props.publication.supplementary || '[]')
...
}[props.publication.status] || 'bg-gray-200 text-gray-700'
